 ...
 */
public class MyWidgetConfig1 extends AppWidgetProvider {

    private static RemoteViews remoteViews;
    protected static Context context;
    public static AppWidgetManager appWidgetManager;
    private static final String ACTION = "com.xfshipan.weather.widgetupdate1";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (ACTION.equals(intent.getAction())) {
            //重新开启两个Service
            if (!isMyServiceRunning(TimeService.class)) {
     ...
     * @param widgetId
     */
    protected static void fillTimeData(RemoteViews remoteViews, int widgetId) {
        ...
        String strMonth = monthFormat.format(calendar.getTime());
        remoteViews.setTextViewText(R.id.tv_realweather_time,
                calendar.get(Calendar.HOUR_OF_DAY) + ":" + calendar.get(Calendar.MINUTE));
        remoteViews.setTextViewText(R.id.tv_realweather_date,
                strMonth + "月" + calendar.get(Calendar.DAY_OF_MONTH) + "日" + " " + dayFormat.format(calendar.getTime()));
        ...
        public void handleMessage(Message msg) {
            ...
            if (taskId == 0) {
                ...
                ComponentName thisWidget = new ComponentName(context,
                        MyWidgetConfig1.class);
                int[] allWidgetIds = appWidgetManager.getAppWidgetIds(thisWidget);
                remoteViews = getRemoteViews();
                for (final int widgetId : allWidgetIds) {
                    fillTimeData(remoteViews, widgetId);
                }
            } else if (taskId == 1) {
                //天气任务
                if (remoteViews == null) {
                        ...
                        ThreadManager.getInstance().createShortPool().execute(new Runnable() {
                            ...
                            public void run() {
                                ...
                                UiUtils.runOnUiThread(new Runnable() {
                                    @Override
                                    public void run() {
//                                    fillWeatherData2(remoteViews, weatherInfo, finalLocalCity, thisWidget);
